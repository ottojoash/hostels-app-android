...
//
// Future<List<Hostel>> fetchHostels({bool? active}) async {
...
//           obj["message"] ?? "The server could not process this request.");
//     }
//     var hostels = obj["hostels"] ?? [];
//     List<Hostel> list = [];
//     if (active == true) {
//       hostels = hostels
//           .where((hostel) => ((int.parse(hostel["active"]) > 0) == true))
//           .toList();
//     } else if (active == false) {
//       hostels = hostels
//           .where((hostel) => ((int.parse(hostel["active"]) > 0) == false))
//           .toList();
//     }
//     hostels.forEach((hostel) => list.add(Hostel.fromJson(hostel)));
//     return list;
//   } else {
//     throw Exception('Failed to load hostels - Received an error ' +
//         response.statusCode.toString());
//   }
// }
//
